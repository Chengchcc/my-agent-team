...
export class ReadTool extends ZodTool {
  schema = z.object({
    ...
    start_line: z.number().int().positive().default(DEFAULT_START_LINE).describe('Starting line number (1-indexed)'),
    end_line: z.number().int().positive().optional().describe('Ending line number (inclusive)'),
    max_lines: z.number().int().positive().default(DEFAULT_MAX_LINES).describe('Maximum lines to return'),
