 ...
 * and a `JobContext` whose `invoke()` and `log()` calls are proxied over
 * stdout NDJSON frames. The parent process services `invoke-req` frames
 * by calling its own ProviderInvoke and writing `invoke-resp` back on stdin.
 */
