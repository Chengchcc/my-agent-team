// src/extensions/frontend.lark/lark/message-parser.ts
...
function fileContent(message: Record<string, unknown>): string {
  try {
...
function extractMessageContent(message: Record<string, unknown>): string {
  // If already normalized (plain text), return directly
  if (isPlainText(message.content)) return message.content
  ...
  if (msgType === 'file') return fileContent(message);
