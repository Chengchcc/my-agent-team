...
export async function listServerResources(client: Client, logger?: Logger): Promise<McpResourceDef[]> {
  try {
    ...
    logger?.debug('mcp', `_listResources failed: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
}

export async function listServerPrompts(client: Client, logger?: Logger): Promise<McpPromptDef[]> {
  try {
    ...
    logger?.debug('mcp', `_listPrompts failed: ${err instanceof Error ? err.message : String(err)}`);
    return [];
  }
}
