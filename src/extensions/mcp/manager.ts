...
export class McpManager {
  ...
  constructor(options: McpManagerOptions, logger?: Logger) {
    ...
    void Promise.allSettled(promises).then(results => {
      for (let i = 0; i < results.length; i++) {
        ...
        if (result.status === 'rejected') {
          this.logger?.debug('mcp', `[McpManager] Failed to connect '${name}': ${result.reason}`);
        }
      }
      this._onReady?.();
    });
    ...
    try {
      ...
      transport.onclose = () => {
        ...
        if (entry?.state.status === 'connected') {
          this.logger?.debug('mcp', `[McpManager] '${config.name}' connection closed unexpectedly`);
          this._servers.set(config.name, {
            ...entry,
            client: null,
            transport: null,
      ...
      this.logger?.debug('mcp', `[McpManager] Connected to '${config.name}': ${tools.length} tools, ${resources.length} resources, ${prompts.length} prompts`);
    } catch (err) {
      if (client) {
  ...
  async disconnectServer(name: string): Promise<void> {
    ...
    if (client) {
      try {
        ...
        this.logger?.debug('mcp', `[McpManager] Error closing '${name}': ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  }
  ...
  private async _listResources(client: Client): Promise<McpResourceDef[]> {
    return listServerResources(client, this.logger);
  }

  private async _listPrompts(client: Client): Promise<McpPromptDef[]> {
    return listServerPrompts(client, this.logger);
  }

  /** Auto-reconnect after unexpected disconnection. Delegates to extracted reconnect logic. */
  private _reconnect(serverName: string): Promise<void> {
    return runReconnect({
      servers: this._servers,
      attempts: this._reconnectAttempts,
      maxReconnectAttempts: this._options.maxReconnectAttempts,
      reconnectDelayMs: this._options.reconnectDelayMs,
      logger: this.logger,
