...
| **agentLoop** | 框架 `span-loop.ts` 内部：单个 Span 内的 step 迭代（调模型→工具执行→循环）。纯实现机制，不是领域概念 | 不是 Loop（多 Span 编排系统）；不与 Loop 系统有任何关系 |
...
| run/span 混用 | ADR 0007 已定：backend 代码→`span`，Web UI→`run`。DB 表 `run`→`span` 待执行 | 完成收敛 |
