...
import { spawnOmpProcess } from "./process.js";

describe("spawnOmpProcess stdout vs orphaned grandchildren", () => {
  ...
  test("child exit code still resolves for a clean run", async () => {
    const proc = spawnOmpProcess(
      { executable: "bash", args: ["-c", "echo done"] },
      { cwd: "/tmp" },
    );
    const lines: string[] = [];
    for await (const line of proc.stdout) lines.push(line);
