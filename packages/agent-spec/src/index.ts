...
export const AgentSpecV2 = z.object({
  ...
  /** Daemon resolves workspace via agentId + daemon roots. */
  ...
  /** Single-shot user input. */
  input: z.string(),

  /** Logical run identifier. */
  runId: z.string().min(1).optional(),

  /** Execution mode. 'reflect' only generated by daemon internally. */
  mode: z.enum(["run", "resume", "reflect"]).default("run"),

  /** Resume payload. Required when mode='resume'. */
  resumeCommand: z
    .object({ approved: z.boolean(), message: z.string().optional() })
    .optional(),

  // M10 fields carried forward
  conversationId: z.string().min(1).optional(),
  senderMemberId: z.string().min(1).optional(),
}).superRefine((v, ctx) => {
  if (v.mode === "resume" && !v.resumeCommand) {
