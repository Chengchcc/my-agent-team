...
export function composeSystemPrompt(p: SystemPromptParts): string {
  return [
    `<workspace>\nRoot: ${escapeClosingTags(p.workspace)}\nFile tools (read, write, edit, grep, glob) use absolute paths relative to this root.\nFor bash, pass cwd='${escapeClosingTags(p.workspace)}' by default; override when the user explicitly requests a different directory.\nToday: ${p.today}\n</workspace>`,
