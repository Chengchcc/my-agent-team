...
反射(reflect)由 **daemon 自己发起,但以 backend 的信号为闸**。一个主 run 成功结束、且它的 spec 标记了需要反射时,daemon 在 `run_done` 里带上 `wantsReflect: true`,并把这个 run 暂存起来;等 backend 完成落库、回发 `run_finalized` 后,daemon 才 `fork` 出一个反射 agent、注入 `reflectionGuidance()` 作为输入、发出 `run_started` 让 backend 建行,然后驱动这个反射 run。反射 spec 会被剥掉 `surfaceContext`,确保它不继承 Lark surface 的工具。
...
**surface 工具按需注入。** 仅当 `start` 带的 `surfaceContext.surface === "lark"`、声明了 `start_new_conversation` 能力、且不是反射模式时,daemon 才动态加载并注入 `start_new_conversation` 工具(它会回调 backend 的 `POST /api/conversations/:id/start-new`)。
