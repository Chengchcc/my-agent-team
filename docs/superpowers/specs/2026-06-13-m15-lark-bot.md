...
> `inbound_message` 是必须状态，不是「持久入站队列」。它同时记录已消费的 Lark `event_id` 与 `message_id`：优先用 `event_id` 防事件重复投递；再用 `message_id` 防同一消息被不同事件路径或重放重复写 ledger / 重复触发 run。

### 4.3 首见 chat：惰性建会话（解析 / 建立流程）

```
on inbound message {event_id, chat_id, chat_type, sender_id, message_id, content}:
    if db.inbound_message.exists_event(event_id) or db.inbound_message.exists_message(message_id):
        return  # 已消费，绝不重复 POST /messages，也绝不重复触发 run
    ...
    db.inbound_message.insert(event_id, message_id, chat_id, conversationId, resp.seq)
```
...
- **入站**：`inbound_message(event_id, message_id)` 让同一 Lark event 或同一 Lark message 不重复写 ledger、不重复触发 run。这个必须做到。
...
**botDisplayName 配置**：`lark-cli` 的 `im.message.receive_v1` 扁平输出不暴露 `mentions[]`，也不需要获取 bot 自身 `open_id`。group @bot 判定改为依赖启动参数 `--bot-display-name`：`content.includes("@" + botDisplayName)`。该值来自 web 创建/编辑 agent 的 Lark Bot 区块；未配置时 lark-bot 必须 fail-closed（p2p 可消费，group 只入账不触发，或进程直接 error，二选一在实现中固定并测试）。

```
1. p2p: chat_type == "p2p" → addressedTo=[agentId]。
2. group: content includes "@" + botDisplayName → addressedTo=[agentId]，否则 []。
3. text/post 的 @ 会被 lark-cli ConvertBodyContent / ResolveMentionKeys 渲染成 @name；interactive 卡片 content 是 raw JSON，MVP 不把卡片当 @bot 触发。
4. parser 单测用真实 event consume 样本 fixture 校准字段路径和 @name 文本。
```
...
| `error` | profile 初始化失败 / botDisplayName 缺失 / event consume 连续失败 |
...
当前 `adapter-sqlite.ts` 已用 `INSERT OR IGNORE` 做 DB 级幂等，但 `service.ts` 仍会无条件 broadcast；M15 必须把 `created` 结果透出，补 service/adapter/http 测试。
...
出站 `render(entry.content)` 至少覆盖三类内容：

```
if content is string:
    try JSON.parse(content) else raw string
if parsed has .text:
    return parsed.text
if parsed is array ContentBlock[]:
    return join text blocks / stringify fallback
else:
...
| `--bot-display-name` | ✅（group 模式下） | — | Lark app 的机器人名称，必须与 Lark 后台设置一致 |
...
| botDisplayName 缺失 | group @ 判定不可用 | fail-closed：进程 error 或 group 仅入账不触发；实现必须固定一种并测试，避免误触发 |
...
| `apps/backend/src/features/conversation/adapter-sqlite.ts` | 修正 `LedgerKind`/row cast 含 `todo`，避免 SSE/ledger 类型不一致继续扩大 |
...
2. `feat(backend): addMember idempotent on existing memberId`
   - ✅ `ConversationPort.addMember` 返回 `{ member, created }`；同 memberId 重复 `POST /members` 不重复 append `member.joined`；返回现有 member；checkpoint 不被重复系统消息污染；现有用例全绿。
