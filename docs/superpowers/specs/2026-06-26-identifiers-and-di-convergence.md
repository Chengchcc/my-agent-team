...
> - `executeAgentRun` 入参袋已增长到 **17 字段**（`run-executor.ts:37-65`，含新 `onRunStatus`）。
...
- **DI 最集中的反例**：收 17 字段大袋子（`:37-65`），函数体内 `agentSvc.getById(agentId)`（`:120`）现查 agent、`new AnthropicChatModel`（`:124`）、`sqliteCheckpointer`（`:175`）、`pipeContextManagers`（`:179`）、`new AgentSession`（`:185`）现造 session，把本该留在 `main.ts` 的 `new` 漏到执行层。
...
- runtime-ops 那 7 处返回 `attemptId` 的点（`runtime-ops/service.ts:42/79/86/273/312/396/412`）**本阶段只加 `attemptSeq` 字段并存**，纯类型扩展、不动查询；**把读写真正切到 seq、删 `attemptId` 留到 Phase 3**（§6 存储落地）。两阶段边界：Phase 0 = 无行为变更纯类型 PR，Phase 3 = 动 SQL + 展示。

---

## 4. Phase 1 — 重构 `executeAgentRun`（枢纽，解决 DI#1 + ID#2 + ID#4）

这是收益最大的一个 PR。改完后 executeAgentRun 从「17 字段袋子 + 函数体造 4 个对象」变成「接收造好的 session + 透传 runId」。
...
`ExecuteAgentRunOpts` 17 字段袋子（`:37-65`）按 [`foundations/identifiers.md`](../../architecture/foundations/identifiers.md) 的「会话 / cron / 编排」三触发源拆成 **稳定能力 `RunDeps`** + **每次调用数据 `RunRequest`** + **触发源判别联合 `RunOrigin`**（详见配套 plan §4 的伪代码）。
...
**关键约束**：`session.state === "waiting"`（等待审批 / 人工 gate，`agent-session.ts:218`）的 session **绝不能被 idle reaper 回收**——现 `finalizeOnce` 已有 `if (session.state !== "waiting")` 判断（`run-executor.ts:204`），回收策略要继承这条；`state === "running"`（正在跑）的也不回收。

### 5.3 跨 run 复用的两条新语义（落地前必须先拍板）

per-run 临时 session 时代不存在、复用后才出现的两个语义缺口，**PR-2 落地前必须先定**，否则并发下丢任务或用错配置（详见配套 plan §5.1a 的伪代码与测试用例）：

1. **同 sessionId 并发 prompt 串行化**：`prompt()` 在 `state === "running"` 时 `throw`（`agent-session.ts` / `create-agent.ts`），第二个并发 run 的 `void session.prompt().catch()`（`run-executor.ts:249`）会被吞成 `error` 静默丢任务。推荐 factory 内按 sessionId 排队串行（队列方案），而非拒绝或另造临时 session。会话连发、cron 与会话撞同一 owner sessionId 是真实并发面。
2. **getOrCreate 二次传入不同 spec**：命中即返回旧 session 会静默丢弃新 spec。命中时须校验 `agentId`/`modelName`/`cwd` 一致，不一致 `throw SessionSpecMismatchError`（issue/cron 改 `:ownerAgentId` 后换 owner、或改 model 时会撞）。

**回归点**：内存泄漏验证（长跑多 session 场景）；waiting/running 态 session 不被误回收的测试；并发 prompt 串行用例；spec mismatch 抛错用例。
...
## 7. Phase 4 — 周边 DI 中高问题（独立小 PR）

这些不在枢纽上，但属同批次债务，各自一个小 PR。**注意依赖分层**：DI#2/4/5/6/7 与枢纽解耦，PR-0 后即可并行（归 PR-5）；但 **DI#8（createOrchestrator 拆分）的 `StepRunnerDeps` 携带 `runDeps: RunDeps`，强依赖 PR-1 重构后的 `RunDeps` 形态**，因此 DI#8 不可与 PR-1 并行，单独归 PR-4（与 ID#5 同 PR，详见 §8 切分）。
...
> **前置约束：`ownerAgentId` 必须是 agent 表里真实存在的行。** 现 `:owner` 是字面量、不经 agent 校验；改 `:${ownerAgentId}` 后该 id 会随 sessionId 流进 `executeAgentRun`，函数体 `agentSvc.getById(agentId)`（`run-executor.ts:120`）按它查 agent，查不到即 throw、run 起不来。落地前须先定 owner agent 的来源：在 setup/migration 阶段写一条保留 agent 种子（如 `agentId = "owner"`），或让 issue/cron 创建时显式指定已存在的 agentId——**不能凭空造字符串**，否则 PR-4 上线后 issue/cron 触发的 run 全炸在 `getById`。种子写入建议走现有 setup provisioner（`main.ts:203`）同一初始化路径。
...
PR-4  Phase 4(DI#8) + Phase 5  createOrchestrator 拆分 + sessionId 格式统一（ID#5）       ← 依赖 PR-1（DI#8 需 RunDeps）
PR-5  Phase 4  周边 DI 中高问题（DI#2/4/5/6/7，不含 DI#8）                                ← 依赖 PR-0，可并行
PR-6  收尾     sessionId 列名迁移 ID#1b（thread_id → session_id，可选，低优先）
```

> DI#8 归属说明：DI#8 拆出的 `StepRunnerDeps` 携带 `runDeps: RunDeps`（PR-1 产物），故不能与 PR-1 并行，与 ID#5 合并进 PR-4；其余 DI 项（DI#2/4/5/6/7）与枢纽解耦，归 PR-5 并行。此切分与配套 plan §9 完全一致。

**强依赖**：PR-B0 → PR-0 → PR-1 →（PR-2 / PR-4）。PR-3 依赖 PR-0 可与 PR-2 并行；PR-5 依赖 PR-0 可并行。
...
- [ ] executeAgentRun 入参从 17 字段袋拆成 `RunDeps` + `RunRequest` + `RunOrigin`。
