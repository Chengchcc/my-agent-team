...
所以 M14.7 **不能**只设计 `{ type:"checkpoint", messages }`，也不能把 checkpoint 简化成 JSON/NDJSON 文件。否则工具审批中断无法恢复、`resume()` 的 interrupt 消费丢失、checkpoint event 审计链断掉。本版引入 **runner-local SQLiteCheckpointer**（§四 4.2）：runner-daemon 内部仍拿到完整 `Checkpointer` 接口，但它写的是 runner-state volume 上的 sqlite，不是 backend DB，也不是 `WorkspaceFS.write()`。
...
class RunnerDaemon {
  ...
  #ws = new Map<string, WorkspaceHandle>();   // agentId → handle cache
  #model: ChatModel;
  #privateRoot: string;
  #sharedRoot: string;
  #stateRoot: string;
  ...
  #handleFor(agentId: string): WorkspaceHandle {
    ...
    if (!h) {
      h = makeWorkspaceHandle({ sharedRoot: join(this.#sharedRoot, agentId), privateRoot: join(this.#privateRoot, agentId) });
...
DB 内部用多张表区分 checkpoint、interrupt、checkpoint event。runner-state 不是普通 AFS workspace，也不通过 `WorkspaceFS` 暴露给 tools；它是 runner 内部持久状态卷。
...
完整 AFS 架构见 [17-agent-file-system](../../architecture/17-agent-file-system.md)。本节只约束 M14.7 必须落地的切片：逻辑绝对路径、`WorkspaceFS` mount table、shared/private/external 三域、`WorkspaceHandle`、runner-daemon 挂载方式，以及删除旧 `workspace: string` 契约。
...
`WorkspaceFS` 不再内置 `#shared/#private` 两槽，而是维护一张 **mount table**。每个挂载点声明：
...
export class WorkspaceFS {
  ...
  #writable(p: string): { backend: WritableBackend; relPath: string } {
    const { mount, relPath } = this.#resolve(p);
    if (!("write" in mount.backend)) throw new WorkspaceAccessError(`read-only mount: ${p}`);
...
消费者迁移后不得再出现 `if (isShared(path)) ... else ...` 的运行时分支；所有读写统一调用 `WorkspaceFS.#resolve`。
...
export function makeWorkspaceHandle(o: { sharedRoot: string; privateRoot: string; sharedPosix?: boolean }): WorkspaceHandle {
...
```ts
const fs = new WorkspaceFS(makeSharedOnlyMounts({ sharedRoot: join(sharedRoot, agentId), sharedPosix: false }));
```

于是 backend 读 `/tmp/a.txt` 这类私有路径时没有任何 mount 命中，直接抛 `WorkspaceAccessError("no mount")`；这比二槽模型里的 `private=undefined` 更泛化，也能覆盖未来 external mount 的访问控制。

### 7.7 消费者迁移：一次性切到 WorkspaceHandle，不保留向后兼容

迁移不是兼容层，而是 **删除裸 `workspace: string` 契约**。本 milestone 结束时，agent/harness/plugins/tools 只接受 `WorkspaceHandle`；runner-stdio 没有使用价值，直接删除，不作为兼容适配层。
...
export interface GenericAgentOptions {
  workspace: WorkspaceHandle;        // 必填，不再接受 string
  model: ChatModel;
  threadId: string;
...
所有调用方必须在进入 harness 前构造好 `WorkspaceHandle`：

- runner-daemon：按 `agentId + roots + mounts` 构造（见 §7.7.2）。
- CLI/测试：用 `makeDevWorkspaceHandle(root)` 显式创建 shared/private 子目录和默认 mounts，不再把 root string 直接传给 harness。
- backend identity handler：只构造 shared-only `WorkspaceFS`，不调用 `createGenericAgent`。
...
class RunnerDaemon {
  ...
  #handleFor(agentId: string): WorkspaceHandle {
    ...
    h = new WorkspaceHandle({ fs: new WorkspaceFS(mounts), privateRoot });
    this.#ws.set(agentId, h);
    return h;
  }
...
1. **`createGenericAgent`**：删除 `workspace: string`；所有内部插件/工具从 `WorkspaceHandle` 派生。
...
4. **`plugin-fs-memory`**：删除 `dir` 参数，改收 `WorkspaceFS`；读写固定使用 `/memory/` 前缀，如 `/memory/2026-06-12.md`。
5. **`plugin-progressive-skill`**：删除 `dir` 参数，改收 `WorkspaceHandle`；默认 skill 路径是 `/skills/`，走 private 根。若未来 skill 迁到共享域，只改 mount table，插件代码不变。
...
8. **backend identity handler**：不读宿主 workspace 目录；按 agentId 构造 shared-only mounts，经 `WorkspaceFS` 读写 `/SOUL.md`、`/USER.md`、`/memory/`。
9. **CLI/测试入口**：必须显式创建 `WorkspaceHandle`；不允许把 string 传进 harness。测试若只需要本地单目录，也要通过 helper 建 shared/private 两个子目录。
...
| 场景 | `WorkspaceFS` 行为 | 等价 code |
|---|---|---|
| `read` 路径不存在 | 命中 mount 后 backend 返回 `null` | `not_found` |
| 无 mount 命中 | 抛 `WorkspaceAccessError("no mount")` | `forbidden` |
| 只读 mount `write` | 抛 `WorkspaceAccessError("read-only mount")` | `unsupported_operation` |
| 路径非法（`..`/空 path/非绝对规范化失败） | 抛 `WorkspaceAccessError("invalid path")` | `forbidden` |
...
`MountWorkspaceProvider` = `infra/workspace.ts` 现状搬迁，但建两域目录：共享域放 `SOUL/USER/TOOLS/AGENTS/BOOTSTRAP/memory`；私有域建空目录。Provider 只负责 provision/teardown 与 roots，不参与 mount table 运行时派发；运行时派发由 `WorkspaceFS` 负责。后续 SandboxWorkspaceProvider/NasBackend 不在本 milestone。
...
| 23 | `createGenericAgent` 收裸路径 | `create-generic-agent.ts` | 删除 `workspace: string` 契约，改为必传 `WorkspaceHandle`；所有调用点同步迁移 |
| 24 | backend identity handler 直读 workspace | `agent/http.ts:85-163` | 改走 shared-only `WorkspaceFS` mounts |
| 25 | workspace 备/拆焊死 `node:fs` | `infra/workspace.ts` | 抽象 `WorkspaceProvider`；两域目录 |
| 26 | agent/skill/memory 直调 `node:fs` | `bootstrap.ts` / plugins / tools | 全量改 `WorkspaceFS`/`WorkspaceHandle`；`workspace-reader.ts` 删除 |
| 27 | 共享前缀硬编码散落 | identity / reflectionGuidance / bootstrap | `isShared()` 仅作审计 helper；真实路由改由 mount table 最长前缀匹配，默认挂载含 TOOLS/AGENTS |
| 28 | `sandbox.ts` 单 root 防护 | `tools-common/src/sandbox.ts:52` | root 扩为 `WorkspaceHandle.posixRoots`（privateRoot + 所有有 posixRoot 的 mount） |
| 29 | workspace 路由只能 shared/private 二选一 | 旧 `WorkspaceFS` 设想 | 本 milestone 直接实现 AFS mount table：prefix/domain/backend/posixRoot，最长前缀匹配，支持 `/mnt/drive/` 等 external provider 测试挂载 |
...
8. **一切结构化 IO 经 `WorkspaceFS`，按 mount table 最长前缀路由**。shared/private 只是默认挂载；external provider 也是挂载项。
9. **能力否定靠类型在场与否表达**。无 mount = forbidden；只读 mount 无 `WritableBackend`。
10. **私有域必须可 POSIX 化**。子进程工具只看 `WorkspaceHandle.posixRoots`；共享/external mount 只有提供 `posixRoot` 时可见。
...
| `packages/workspace-fs/` | `ReadableBackend`/`WritableBackend`、`WorkspaceFS` mount table、`MountEntry`、`LocalBackend`、`FileBackend`、`MemoryBackend`（测试）、`WorkspaceHandle`、`makeDefaultMounts`、`isShared`（审计 helper，含 TOOLS/AGENTS） |
| `packages/workspace-provider/` | `WorkspaceProvider` + `MountWorkspaceProvider`（两域） |
| `packages/harness/src/create-generic-agent.ts` | 删除 `workspace: string`，改为必传 `WorkspaceHandle`；接收 framework `Checkpointer` |
| `packages/harness/src/bootstrap.ts` | 改 `WorkspaceFS.read` 读共享域文件；删除 `workspace-reader.ts` |
| `packages/plugin-fs-memory/` / `plugin-progressive-skill/` / `tools-common` | 全量改 WorkspaceFS/WorkspaceHandle；bash/grep/glob 走 `posixRoots` |
| `packages/tools-common/src/sandbox.ts` | 允许 `WorkspaceHandle.posixRoots` 多前缀 |
| `apps/backend/src/features/agent/http.ts` | identity handler 改 WorkspaceFS（shared only） |
...
8. `feat(workspace-fs): AFS mount table + capability facade + default mounts incl TOOLS/AGENTS`
   - ✅ 最长前缀匹配、同 prefix 后注册覆盖、无 mount/只读 mount 错误、backend 无 private 触私有抛错、shared 文件归属测试。
9. `refactor(harness): require WorkspaceHandle in createGenericAgent`
   - ✅ 所有 `createGenericAgent` 调用点传 `WorkspaceHandle`；仓库内无 `workspace: string` harness 契约残留。
10. `refactor(workspace): migrate bootstrap/plugins/tools to WorkspaceFS`
...
- [ ] WorkspaceFS mount table 最长前缀匹配：`/memory/` 覆盖 `/`；`/mnt/drive/` external mock 可挂载并读写。
- [ ] 同 prefix 后注册覆盖前注册；包含 `..` 的 path 被 `WorkspaceFS` 拒绝。
- [ ] backend `WorkspaceFS` 只注册 shared mounts，读私有 path 因无 mount 抛 `WorkspaceAccessError`。
- [ ] `createGenericAgent` 只接受 `WorkspaceHandle`；仓库内不存在 `createGenericAgent({ workspace: "..." })` 或 `WorkspaceInput = string | WorkspaceHandle`。
- [ ] bash/grep/glob：只放行 `WorkspaceHandle.posixRoots`；mount 无 `posixRoot` 时对子进程不可见。
...
| **Daemon** | 多 agent cache | ✅ agent-scoped（`--agent-id`，单 WorkspaceHandle，单 Checkpointer） |
