...
interface MemoryLifecycleConfig {
  ...
  enableContradictionMerge: boolean // false (MVP)
...
memory: {
  lifecycle: {
    ...
    contradictionTopK: 3,
    enableContradictionMerge: false,
...
MVP 上线时只开启 §4 步骤 1-3 + §6 衰减 + §7 加固。矛盾仲裁与 prune 留开关,在生产观察一周后再打开。
