...
10. **产品工具的呈现形状收敛在 `packages/api-contract`（2026-09-24）。** oma 产品工具（todo / ask / approval）由 backend 注入、执行、鉴权后以标准 Run SSE 事件下发，飞书端与 Web 各自渲染一次，**任何一端都不得重新解释工具输出或从 `text_delta` 猜**。跨端共用的载荷形状（本期是 `OmaTodoItem` / `OmaTodoStatus`）定义在 `packages/api-contract/src/sse.ts` 并同时被 Web reducer 与卡片 reducer 引用——此前 `todo_update.payload.items` 是 `z.array(z.unknown())`，两端各写一份结构，卡片侧把生产方的 `done` 写成 `completed`，已完成的步骤在卡上整条消失。**状态词表属于生产方**（oma todo 插件：`pending | in_progress | done | cancelled`），消费端自造词表就是静默丢数据。
