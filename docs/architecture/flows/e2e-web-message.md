...
Web 不再维护 draft/run 状态。`ConversationMessageRevision` 携带 `messageId`（assistant 消息用 `assistantMessageId(runId)`，human 消息用 `s-${seq}`），Web reducer 按 `messageId` upsert 到 `items[]`（元素是 `kind: "message"` 的 UiItem）。同一 run 的多轮增量投影共用同一个 `messageId`——每次账本 SSE 到达时 `upsertAuthoritative` 找到同 id 就替换，UI 自然刷新。
...
5. Agent 产出 → message 事件经 `onRunMessage` 直写账本（非消息事件才进 EventLog）。
6. assistant message → `buildAssistantRevision` → `ConversationMessageRevision`（`{ messageId, state, role, text/blocks, runId }`）经 `appendAssistantMessage` 落账本。
7. Revision → Web reducer `parseRevision` → `UiItem`（`kind: "message"`，`content: ConversationMessageRevision`）。
