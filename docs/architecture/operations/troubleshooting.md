...
summary: "这一页把常见故障按「症状 → 该去哪条事实线查 → 根因方向」组织起来。核心心法是：先判断问题落在哪一层——是对话事实（账本）错了，还是 projection/流（projection_messages 表、SSE）错了，还是执行(Runner / 事件日志)错了——再顺着对应的存储和不变式去定位，而不是一上来就乱翻日志。"
depends_on:
...
这一页把常见故障按「症状 → 该去哪条事实线查 → 根因方向」组织起来。核心心法是：先判断问题落在哪一层——是对话事实（账本）错了，还是 projection/流（projection_messages 表、SSE）错了，还是执行(Runner / 事件日志)错了——再顺着对应的存储和不变式去定位，而不是一上来就乱翻日志。
...
- projection_messages 表（projection_messages）、SSE 流是投影；
...
| 症状 | 先看哪里 | 根因方向 |
|------|----------|----------|
| 某成员看不到本该有的消息 | 该成员的 projection_messages 表（thread_id = conversationId:memberId） | `broadcastMessage` fan-out 失败；thread 推导错；账本其实有、投影缓存没跟上 |
