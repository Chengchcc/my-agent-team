...
**发现路径**：固定 `${dir}/*/SKILL.md`（一层嵌套）。**不支持**深层 glob——目录结构就是 skill 的命名空间，深层嵌套会让 name 冲突管理混乱。
...
```ts
// Replaced with logical path (e.g. /skills/pdf-extract)
