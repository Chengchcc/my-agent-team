...
>
> 关联：[Backend — Team Runtime](./12-backend.md)（触发方）· [AgentSpec](./13-agent-spec.md)（启动契约）· [EventLog](./14-event-log.md)（事件可见性边界）· [plugin-fs-memory](./06-plugin-fs-memory.md)（workspace 级存储）· [M14.3 Reflection 后置](../superpowers/specs/2026-06-10-m14.3-reflection-post-run.md)（锁与 run 边界的先例）。
...
**M14.7 落地决策**：daemon 是 **agent-scoped**（一个 daemon 绑定一个 agent，`--agent-id` 启动参数）。dev 下 `DevRunnerRegistry` lazy spawn 本地 daemon；prod 下 `ProdRunnerRegistry` 只解析已有 endpoint。daemon 生命周期由 registry 管理，不在 dev.sh 里手动 spawn/pkill。
...
1. **D19 污染**：辅助 run 的 tool_use / text 被投影进主 conversation ledger，与主 run 产出混在一起
...
2. 必须是**独立 runId / 独立可见性**（否则混进主 turn，造成 D19 污染和前端折叠）。
...
2. **反思走独立 runId / 独立可见性边界。** [D19 投影](./12-backend.md)、前端 turn 分组都按"主 run 的可见事件流"切分，反思产出 ≠ turn 产出，必须物理隔离。今天靠独立进程 + 独立 runId，常驻后靠 daemon 多路复用时的 per-event runId 标注，**需求不变、且变得更刚性**。
