...
summary: "持久状态分布在三处：backend.db 拥有 agents、对话、成员、账本、projection_messages 表和 issue(M18.1)；events.db 拥有运行、尝试、事件日志和各类 ops/健康表；Runner 本地的 checkpointer.sqlite 拥有 resume checkpoint。"
depends_on:
...
持久状态分布在三处：backend.db 拥有 agents、对话、成员、账本和projection_messages 表；events.db 拥有运行、尝试、事件日志和各类 ops/健康表；Runner 本地的 checkpointer.sqlite 拥有 resume checkpoint。
...
assistant 消息现在经 `appendAssistantMessage` 直写账本（不再通过增量projection bridge从 EventLog 派生）。streaming 修订和 terminal 修订共享同一个 `messageId`，端按 `messageId` upsert。
