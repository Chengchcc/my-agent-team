...
- `startMainRun(runId, threadId, spec)`：INSERT `run`（status `running`）→ `#beginAttempt`。
...
case "event": {
  ...
  for (const fn of this.#onRunEvent) {
    try { await fn(threadId, runId, event); }
    catch (err) { console.error(err); }   // 监听器失败只记日志，不影响运行
  }
}
```
...
收割间隔 = `config.reaperIntervalMs`（>0 时），否则取 `min(heartbeatTimeoutMs/2, 30_000)`。每个 tick 由 `#reaping` 守门，避免重入。`#reapStaleRuns` 扫描 `ended_at IS NULL` 的 attempt：`age = now - heartbeat_at`，`age < heartbeatTimeoutMs` 视为新鲜、跳过；僵死的则事务性地把 run 置 `interrupted`、attempt 置 ended，并尽力 append 一条 `interrupted` 事件，再跑 `onRunComplete`（收割器里不 await）。daemon 运行没有后端 pid，所以**心跳超时是唯一的存活信号**。
