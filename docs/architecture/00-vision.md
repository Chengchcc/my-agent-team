...
```text
L6 Surfaces    用户/IM 接入面：frontend web、IM bot（[Lark](./18-im-adapter.md) 等）、CLI
                ↓ 通过 HTTP/SSE 调用
...
9. **Wiki-first documentation** — 架构文档是 wiki，不是代码走读。读者不需要看源码就能理解概念。概念先行，代码佐证。除 vision 外不加里程碑标签——永远描述当前态
...
- [17-agent-file-system.md](./17-agent-file-system.md) — Agent File System：agent 看到的虚拟文件树，AgentFS + mount table + domain 模型

**Surfaces**

- [18-im-adapter.md](./18-im-adapter.md) — IM Adapter：把一个 agent 接成 Lark bot（per-agent L6 surface）

**Observability**

- [14-event-log.md](./14-event-log.md) — EventLog 事实源（含 delta vs 持久化通道边界）
- [02-framework.md](./02-framework.md) — AgentEvent 全量 envelope + 双通道分类
