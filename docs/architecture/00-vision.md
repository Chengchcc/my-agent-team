...
| **M9** | **Durable Runs：run 子进程执行 + [EventLog](./13-event-log.md) 事实源(直连 PG) + SSE 投影解耦(Last-Event-ID 续读) + cancel 透传 + backend 重启重新发现 + 迁移台账统一 + M1/M3 债务** | 🚧 |
...
- [09-harness-generic.md](./09-harness-generic.md) — Generic harness（file-driven 通用 harness）
- [10-harness-vs-framework.md](./10-harness-vs-framework.md) — Framework vs Harness vs Backend 边界

**Backend & Wire**

- [11-backend.md](./11-backend.md) — Backend Agent 托管服务（HTTP server + agent/thread 管理 + runner pool + M9 Durable Runs）
- [12-agent-spec.md](./12-agent-spec.md) — AgentSpec wire schema（Backend ↔ Runner 契约）
- [13-event-log.md](./13-event-log.md) — EventLog 执行事件事实源（M9，从 Checkpointer 拆出，SSE 投影数据源）
