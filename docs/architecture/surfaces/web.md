...
summary: "Web 端是浏览器里的对话界面。它只消费一条 SSE——会话/账本流——在 conversation-reducer 里按 ConversationMessageRevision 的 messageId upsert 到 messages[]。不再有草稿、运行阶段、中断状态——busy 从 open message 的 state 字段推导。"
depends_on:
  - conversation.ledger
...
Web 端是浏览器里的对话界面。它只消费一条 SSE——会话/账本流——在 conversation-reducer 里按 ConversationMessageRevision 的 messageId upsert 到 messages[]。不再有草稿、运行阶段、中断状态——busy 从 open message 的 state 字段推导。

## 这页解决什么问题

用户既要看到持久的对话历史，又要看到进行中的运行产出。Web 不再维护独立的 draft/run 状态——所有输出（人类消息、assistant 流式产出、最终答案、todo 快照）都作为 ConversationMessageRevision 经账本 SSE 到达，按 messageId upsert。实时感来自 revision 的 `state: "streaming"` 增量更新。
...
Action 联合（精确）：`bootstrap`、`ledger/member`、`ledger/message`、`send`、`ledger/conn`、`toggleTriggerMode`、`todo/update`。
...
- `member.joined` / `member.left` → 系统通知。
...
`ledger/message` reducer 分支调用 `parseRevision(a.seq, a.content)`，用 `revision.messageId` 作为 `UiMessage.id`。`upsertAuthoritative` 按 id 查找同消息——找到就替换，否则对己方消息替换最近一条乐观消息，再否则追加。

`busy` 不再由 run phase 推导。`isBusy()` 纯函数检查 messages 中是否有 agent 消息 `state === "streaming"` 或 `state === "waiting"`：
...
- `upsertAuthoritative`：有同 messageId 就替换；否则对自己的消息替换最近一条乐观消息（`opt-` 前缀）；再否则追加。revision 的 messageId 保证同 run 增量/终端写入 upsert 到同一 UiMessage。
