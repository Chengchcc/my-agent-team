...
│   └── ...
└── .index.json            # facts 的元数据索引（标题 / tags / ts）
```
...
- `.index.json` 是 plugin 维护的 facts 索引，纯派生数据，删了也能从 `facts/*.md` 重建
...
| `memory_write` | `{ content: string; tags?: string[] }` | `{ path: string }` 写入路径 | 在 `facts/${ts}-${slug}.md` 新增文件 + 更新 `.index.json` |
| `memory_search` | `{ query: string; limit?: number }` | `Array<{ path, title, tags, snippet }>` | 无 |

### memory_search v1：纯 keyword

按你的决议（Q3 不做 embedding），v1 实现：

1. 加载 `.index.json` 拿到所有 facts 的 `{ path, title, tags, body_keywords }`
2. 对 `query` 做简单 tokenize（小写 + split whitespace + 去停用词）
3. 对每个 fact 算 score = `tag_hit * 3 + title_hit * 2 + keyword_hit * 1`
4. 取 top `limit ?? 5`，返回每条 fact 的前 200 字 snippet

embedding v2 留 future work——届时引入 `@my-agent-team/embeddings` 适配包，不动 plugin 接口。
...
async function memoryWrite({ content, tags }) {
  ...
  const filepath = path.join(dir, 'facts', filename);

  const frontmatter = `---\nts: ${ts}\ntags: ${JSON.stringify(tags ?? [])}\n---\n`;
  await fs.writeFile(filepath, frontmatter + content);

  await updateIndex(filepath, content, tags);
  return { path: filepath };
}
```

`.index.json` 用原子写（tmp + rename）保持与 [fileCheckpointer 同款纪律](./04-checkpointer.md#filecheckpointer)。
...
- **不内置 embedding** — v1 纯 keyword search。embedding 留独立适配包
