...
1. 不同领域之间 ≥ 80% 重叠（都需要 memory、skills、permission、bootstrap）— 按 [Framework vs Harness §反模式](./11-harness-vs-framework.md) 的"≥70% 重叠 → 加参数"原则，应合并而非平行
...
| 步骤 | 谁做 |
|---|---|
| 创建 `agentId` 并存元数据 | [Backend](./12-backend.md) |
...
harness 包**永远不引入任何网络/transport 依赖**。详见 [12-backend.md §跨进程隔离](./12-backend.md#跨进程隔离--通过-runner-透明套上)。
...
具体实现见 [10-harness-generic.md](./10-harness-generic.md)。