  ...
  return new Elysia()
    ...
    .post(
      ...
      async ({ body, set }) => {
        ...
        const now = Date.now();
        svc.port.createConversation({ conversationId, triggerMode: body.triggerMode ?? "mention", createdAt: now });
    ...
    .get("/api/conversations/:id", ({ params: { id } }) => {
      ...
      return { conversationId: conv.conversationId, triggerMode: conv.triggerMode, hopCount: conv.hopCount, title: conv.title, members };
    })
    ...
    .post(
      ...
      async ({ params: { id: conversationId }, body }) => {
        ...
        await svc.addMember({ conversationId, memberId, kind: body.kind, agentId: body.agentId, userRef: body.userRef, displayName: body.displayName });
    ...
    .post(
      ...
      async ({ params: { id: conversationId }, body, set }) => {
        try {
          const result = await svc.startNewConversationForSurface({ oldConversationId: conversationId, ...body });
          set.status = 201;
          return result;
        } catch (err) {
