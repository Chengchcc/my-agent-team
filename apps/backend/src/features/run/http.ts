...
import { getSession, removeSession } from "./session-registry.js";
 ...
 *  Cleans up the session after resume unless it re-enters waiting state. */
export function resumeRoute() {
  return async (req: Request, runId: string): Promise<Response> => {
    ...
    try {
      ...
      // Cleanup if run completed; keep alive if interrupted again
