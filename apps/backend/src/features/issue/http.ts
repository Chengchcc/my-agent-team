  ...
  return new Elysia()
    ...
    .patch(
      ...
      ({ params: { id: issueId }, body }) => {
        try {
          ...
          if (err instanceof IssueNotFoundError) return Response.json({ error: err.message }, { status: 404 });
          if (err instanceof ValidationError) return Response.json({ error: err.message }, { status: 400 });
          throw err;
        }
      },
      ...
      ({ params: { id: issueId }, body }) => {
        try {
          ...
          if (fromStatus && BACKWARD_EDGES.some((e) => e.from === fromStatus && e.to === toStatus)) {
            return Response.json({ error: `backward transition ${fromStatus}→${toStatus} requires review-decision` }, { status: 409 });
          }
          ...
          if (fromStatus && ORDER.indexOf(toStatus) > ORDER.indexOf(fromStatus) && toStatus !== "done") {
            void onIssueStarted?.(updated).catch((e) => console.error(`[orchestrator] startStep failed for ${issueId}: ${String(e)}`));
          }
          return { issue: updated };
        } catch (err) {
          if (err instanceof IssueNotFoundError) return Response.json({ error: err.message }, { status: 404 });
          if (err instanceof IllegalTransitionError) return Response.json({ error: err.message }, { status: 409 });
          throw err;
        }
      },
    ...
    .post(
      ...
      ({ params: { id: issueId }, body, set }) => {
        if (!svc.port.getIssue(issueId)) return Response.json({ error: "issue not found" }, { status: 404 });
        let fromStatus = "";
        if (body.spanId) {
          const origin = opsStore.getSpanOrigin(body.spanId);
          if (!origin) return Response.json({ error: "run not found" }, { status: 409 });
          if (origin.issueId && origin.issueId !== issueId) return Response.json({ error: "run/issue mismatch" }, { status: 409 });
          fromStatus = origin.fromStatus;
        }
        const { row, replay } = deliverableSvc.submit({ issueId, fromStatus, kind: body.kind, fields: body.fields, ref: body.ref, spanId: body.spanId });
        if (!replay) {
          emitIssueEvent(opsStore, issueId, "deliverable.submitted", { kind: row.kind, deliverableId: row.deliverableId, spanId: row.spanId ?? null, ref: row.ref ?? null });
        }
    ...
    .post(
      ...
      async ({ params: { id: issueId }, body }) => {
        const issue = svc.port.getIssue(issueId);
        if (!issue) return Response.json({ error: "Not found" }, { status: 404 });
        if (issue.status !== "in_review") return Response.json({ error: `issue not awaiting review (status=${issue.status})` }, { status: 409 });
        try {
          ...
          emitIssueEvent(opsStore, issueId, "human.decided", { decision: "reject", note: body.note });
          emitIssueEvent(opsStore, issueId, "status.advanced", { from: "in_review", to: "in_progress", by: "rework" });
          try {
            const d = deliverableSvc.submit({ issueId, fromStatus: "in_review", kind: "rework_feedback", fields: { note: body.note! } });
            emitIssueEvent(opsStore, issueId, "deliverable.submitted", { kind: "rework_feedback", deliverableId: d.row.deliverableId, spanId: null, ref: null });
            ...
            try {
              ...
              return Response.json({ error: "rework failed; issue returned to in_review", issue: reverted }, { status: 502 });
            } catch (rollbackErr) {
              console.error(`[orchestrator] rollback failed for ${issueId}: ${String(rollbackErr)}`);
              return Response.json({ error: "rework failed and rollback failed", issue: updated }, { status: 500 });
            }
          }
          return { issue: updated };
        } catch (err) {
    ...
    .get("/api/issues/:id/timeline", ({ params: { id: issueId } }) => {
      if (!svc.port.getIssue(issueId)) return Response.json({ error: "Not found" }, { status: 404 });
    ...
    .get("/api/issues/:id/detail", ({ params: { id: issueId } }) => {
      ...
      const runs = origins.map((o) => {
        ...
        return { spanId: o.spanId, fromStatus: o.fromStatus, agentId: o.agentMemberId, createdAt: o.createdAt, status: run?.status ?? "unknown", endedAt: run?.endedAt ?? null };
      });
    ...
    .get("/api/issues/events", ({ request }) => {
      ...
      return sseResponse(stream, (row) => ({ id: (row as IssueRow).issueId, event: "issue", data: row }), request.signal);
    })
    .get("/api/issues/:id/timeline/events", ({ request, params: { id: issueId } }) => {
      ...
      return sseResponse(stream, (e) => ({ id: String((e as IssueEvent).seq ?? ""), event: "issue-event", data: e }), request.signal);
    });
}
