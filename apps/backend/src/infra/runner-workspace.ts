...
export function safeRunnerAgentId(id: string): string {
  if (!/^[a-zA-Z0-9_-]+$/.test(id)) {
    throw new Error(`invalid runner agentId: "${id}" — contains characters outside [a-zA-Z0-9_-]`);
  }
  return id;
}

/** Single source of truth for all runner directory layout. */
export function runnerWorkspacePaths(dataDir: string, agentId: string): RunnerWorkspacePaths {
...
export async function ensureRunnerWorkspace(paths: RunnerWorkspacePaths): Promise<void> {
...
async function copyIfMissing(src: string, dst: string): Promise<void> {
  try {
    ...
    if (
      ...
      (err.message.includes("already exists") || err.message.includes("EEXIST"))
    )
      return;
    throw err;
  }
...
async function copyDirFilesIfMissing(srcDir: string, dstDir: string): Promise<void> {
...
const IDENTITY_FILES = ["SOUL.md", "USER.md", "BOOTSTRAP.md", "TOOLS.md", "AGENTS.md"] as const;
...
export async function migrateLegacyWorkspaceToShared(
  ...
  for (const file of IDENTITY_FILES) {
    await copyIfMissing(path.join(legacyWorkspacePath, file), path.join(sharedRoot, file));
  }
  ...
  await copyIfMissing(path.join(legacyMemDir, "MEMORY.md"), path.join(sharedMemDir, "MEMORY.md"));

  // legacy/memory/facts/*.md → shared/memory/facts/*.md
  await copyDirFilesIfMissing(path.join(legacyMemDir, "facts"), sharedFactsDir);
  ...
  for (const entry of legacyEntries) {
    ...
    if (!entry.endsWith(".md")) continue;
    if (entry.includes("..") || entry.includes("/") || entry.includes("\\")) continue;
    await copyIfMissing(path.join(legacyMemDir, entry), path.join(sharedFactsDir, entry));
  }
}
...
export async function purgeRunnerWorkspace(opts: {
  ...
  if (runnerRoot === runnersRoot || !runnerRoot.startsWith(runnersRoot + path.sep)) {
