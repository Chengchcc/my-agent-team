...
import type { AgentRuntimeStatus, RunOpsDetail, RunOpsListItem } from "./api";
import {
  diagnoseRun,
  diagnoseRunListItem,
  hasSurfaceError,
...
function makeDetail(
  overrides: Partial<RunOpsDetail["run"]> & {
    transport?: string;
...
describe("diagnoseRun", () => {
  ...
  test("running + noop → detached_waiting_reaper", () => {
    ...
    expect(diagnoseRun(d, HEARTBEAT_MS)).toEqual({
      kind: "detached_waiting_reaper",
      owner: "backend_runner_link",
    });
  });
  ...
  test("running + reattach_failed ops → surface_projection_failed", () => {
    ...
    expect(diagnoseRun(d, HEARTBEAT_MS)).toEqual({
      kind: "surface_projection_failed",
      owner: "surface",
    });
  });
...
describe("diagnoseRunListItem", () => {
  ...
  test("running + noop → detached_waiting_reaper", () => {
    ...
    expect(diagnoseRunListItem(item, HEARTBEAT_MS)).toEqual({
      kind: "detached_waiting_reaper",
      owner: "backend_runner_link",
    });
  });
...
describe("health predicates", () => {
  test("isStaleRun detects stale heartbeat", () => {
    ...
    expect(
      isStaleRun(makeListItem({ status: "succeeded", heartbeatAgeMs: 120_000 }), HEARTBEAT_MS),
    ).toBe(false);
  ...
  test("isDetachedRun detects noop/detached", () => {
    ...
    expect(isDetachedRun(makeListItem({ runnerTransport: "noop", status: "succeeded" }))).toBe(
      false,
    );
  });
  ...
  test("isUnhealthyAgent flags degraded/offline runners", () => {
    ...
    expect(isUnhealthyAgent({ ...base, runner: { ...base.runner, checkpointerOk: false } })).toBe(
      true,
    );
    expect(isUnhealthyAgent({ ...base, runner: { ...base.runner, workspaceOk: false } })).toBe(
      true,
    );
    expect(isUnhealthyAgent({ ...base, runner: { ...base.runner, status: "offline" } })).toBe(true);
    expect(isUnhealthyAgent({ ...base, runner: { ...base.runner, status: "degraded" } })).toBe(
      true,
    );
  });
  ...
  test("hasSurfaceError detects non-running surfaces", () => {
    ...
    expect(
      hasSurfaceError({
        ...base,
        surfaces: {
          lark: { status: "degraded", lastSeenAt: Date.now(), lastError: null, counters: {} },
        },
      }),
    ).toBe(true);
  });
