    ...
    <div className="flex items-start gap-3 border-l-2 border-border pl-3 relative">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="font-mono text-xs font-semibold text-foreground">{o.kind}</span>
...
export function RunOpsTimeline({ ops }: { ops: RunOpsDetail["ops"] }) {
  if (ops.length === 0)
    return <p className="text-muted-foreground text-sm">No diagnostic events recorded.</p>;
  return (
