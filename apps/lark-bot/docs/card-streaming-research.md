...
> **状态：已废弃（superseded）。** 本文描述的 `run-delta-watcher` / `card-sender` / `card-renderer` 路径从未进入生产代码。实际出站路径走 `sse-watcher.ts`，消费 conversation ledger SSE，经 `render.ts` 转为纯文本发送；现状见 [飞书端](../../../docs/architecture/surfaces/lark.md)。卡片流式渲染方案保留为未来参考，但当前架构不沿此方向。
>
