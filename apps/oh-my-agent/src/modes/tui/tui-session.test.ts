...
describe("tui session (headless, fake provider)", () => {
  ...
  test("slash commands: help lists, unknown hints, /model lists catalog", async () => {
    ...
    try {
      ...
      // 25 static commands (incl. /mcp, /skill, /workflow, /memory,
      // /permission, /paste, /compact, /goal); the pinned agent dir
      // guarantees zero auto-registered skills.
      expect(registered).toEqual([25]);
